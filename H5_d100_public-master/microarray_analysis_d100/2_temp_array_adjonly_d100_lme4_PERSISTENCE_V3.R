...
# save path 
datapath = here("microarray_analysis_d100/generated_data_V3_adjonly/"); dir.create(datapath)
figpath = here("microarray_analysis_d100/figures_V3_adjonly/"); dir.create(figpath)
...
eddf = edf %>% 
  ...
  filter(subject.id %in% c("H5N1-005", "H5N1-003", "H5N1-006")) %>% 
...
exprs_mat = eddf[ ,c(1, 4:ncol(eddf))] %>%
  # subset to adjuvant subjects from metadata above
...
#   p = ggplot(hyp_plot2, aes(x = reorder(ID, -log10(p.adjust)), y = -log10(p.adjust), size = gene_ratio)) + 
#     geom_point(color = 'red') +  
#     ggtitle(paste0("AS03 d100 FC associated \n GP ", i)) + 
#     xlab(" ") + 
#     ylab("hypergeometric test \n -log adjusted p value") + 
#     theme_gray() + 
#     coord_flip() 
...
p = ggplot(rsub, aes(x = logFC , y =  -log10(adj.P.Val), label = gene, color = annotation)) +
  ...
  xlab("adjuvanted day 100 fc") + ylab("-log adjusted p value") +
