...
run.id = "R3"
run.ver = "v4"
fn.run = file.path(dn.enet, run.ver, glue::glue("{run.id}_{run.ver}_d42.Robj"))

load(fn.run, verbose = T)
al = c(0.1, 0.2, 0.3, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0)
for(z in al){
        a = z
        ...
        dn.fig = file.path(PROJECT_DIR, "FIGURES/eNet/R3_d42")
...
# scatter plot of predictin performance
        ...
        p0 = ggplot(DF.plot, aes(measured, pred_mean)) +
          ...
          xlab("MN titer, measured at day 42") +
