...
for(N_idx in 1:ncol(N_cmb)){
        ...
        max_genes_0_exp <- pbmc_exp_data[max_genes_0, common_subj]
        max_genes_0_z_score <- get_score(max_genes_0_exp)
        max_genes_0_z_score <- max_genes_0_z_score[N_hold]
...
write.table(val_result, file.path("RESULTS", "rspo3", "validation_result_diff_all_with_38.txt"), sep = "\t", row.names = F)
