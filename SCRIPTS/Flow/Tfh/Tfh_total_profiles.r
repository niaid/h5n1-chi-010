...
DF = DF %>% select(subject, time, matches("_count")) %>% 
...
pop_order = c("Lymphocytes", "Neutrophils", "Tfh")
pop.in = c("Lymphocytes", "Neutrophils", "Tfh")
...
ggsave(file.path(dn.fig, "Tfh_profiles_flow10_counts_generated_change_average.png"), w=5.5, h=5)
...
ggplot(DF.mean %>% filter(population %in% pop.in), aes(time, count.mean, group=Adjuvant, col=Adjuvant)) +
  ...
  geom_errorbar(aes(ymin = count.mean-count.sd, ymax = count.mean+count.sd), width=0.2) +
  theme_bw()
ggsave(file.path(dn.fig, "Tfh_profiles_flow10_counts_generated_average.png"), w=5.5, h=5)
