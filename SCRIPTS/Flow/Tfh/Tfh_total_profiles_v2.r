...
ggsave(file.path(dn.fig, "Tfh_count_generated_NL.ratio_change_average_profiles.png"), w=5.5, h=5)
...
ggplot(DF.mean %>% filter(population %in% pop.in), aes(time, count.mean, group=Adjuvant, col=Adjuvant)) +
  ...
  theme_bw() +
  theme(strip.background = element_blank())
ggsave(file.path(dn.fig, "Tfh_count_generated_NL.ratio_abs_average_profiles.png"), w=5.5, h=5)
