...
dn.in = file.path(PROJECT_DIR, "RESULTS/Microarrays/PBMC/baseline/")
...
for(m in 1:max(net$colors)) {
  fn.gene = file.path(dn.out, sprintf("GE.pbmc_d0_WGCNA_Gb%02d_genes.txt",m))
...
colnames(ME) = sprintf("Gb%02d", 1:length(mod.ngenes))
...
fwrite(df.ME, file.path(dn.out, "GE.pbmc_d0_WGCNA_ME_scores.txt"), sep="\t")

df.summary = data.frame(module=names(ME), ngenes=as.numeric(mod.ngenes), iqr, pc1.ev) %>% 
  tibble::remove_rownames()
fn.summary = file.path(dn.out, "GE.pbmc_wgcna_modules_summary.txt")
