...
dn.mod = file.path(PROJECT_DIR, "RESULTS/Microarrays/PAXgene/baseline/")
...
names(res) = sprintf("GbWB%02d",1:K)
res[sapply(res,is.null)] = NULL
colnames(tmod.pv) = names(res)

dn.out = file.path(PROJECT_DIR, "RESULTS/Microarrays/PAXgene/baseline/")
fn.rds = file.path(dn.out, sprintf("GE.pax_d0_WGCNA_BTM.%s.HG.pv.rds",mset))
saveRDS(res, fn.rds)
fn.tmod = file.path(dn.out, sprintf("GE.pax_d0_WGCNA_BTM.%s.HG.pv.fdr.txt",mset))
