...
mn.ord = match(rownames(dat2),rownames(mn.d42))
...
hm.t = rowAnnotation(`A/Indonesia d42` = row_anno_barplot(log2(mn.d42$A.Indonesia), axis = T,
                                                          bar_width = 0.8,
...
#                   show_column_names = F, width=unit(1,"cm"))
# set.seed(122)
fn.fig = file.path(PROJECT_DIR, "FIGURES/GE_subject_patterns_cor_flow_ip10_titers_sex.pred_incl.s10.pdf")
pdf(fn.fig)
...
fn.fig = file.path(PROJECT_DIR, "FIGURES/GE_subject_patterns_cor_flow_ip10_titers_sex.pred_incl.s10.png")
...
fwrite(df.adj_predict, file.path(dn.pred, "adjuvant_predicted_subjects.txt"), sep="\t")
