...
data = read.table(file.path(path_infile,"eNet_InputData_r6_predicted_status.txt"), header=T, stringsAsFactors=F, sep="\t")
run_index = 6 # integer value in the 1:9 range
...
predictor_rm = c("Gp09", "Gp10", "GbWB11")
...
result = eNetXplorer(x=x,y=y,family="gaussian",alpha=seq(0,1,by=0.1),seed=123)# ,nlambda.ext=1000,n_run=1000, n_perm_null=250)
                                                                               # comment
# these options to use default options.
save(result,file=file.path(path_outfile, paste0(label,"_",version,"_noGbWB11_predicted.Robj")))
summaryPDF(result,path=path_outfile,filename=paste0(label,"_",version,"_noGbWB11_predicted.pdf"))
