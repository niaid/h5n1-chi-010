...
data = read.table(file.path(path_infile,"eNet_InputData_r6.txt"), header=T, stringsAsFactors=F, sep="\t")
run_index = 6 # integer value in the 1:9 range
endpoint = "titer_I_MN_d42"
...
save(result,file=file.path(path_outfile, paste0(label,"_",version,"_d42.Robj")))
summaryPDF(result,path=path_outfile,filename=paste0(label,"_",version,"_d42.pdf"))
