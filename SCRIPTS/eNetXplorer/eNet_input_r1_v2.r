...
# FLOW PATTERN SCORES
ver = "_v2"
fn.fp = file.path(PROJECT_DIR, "RESULTS/Flow_10c", ver, "DeltaThr0.5_DonorThr0.3_PercentOfParent_CORpearson_cutreeHybrid_deepSplit0_minClusterSize31_DonorScores.txt")
df.fp = fread(fn.fp) %>% 
...
dn.out = file.path(PROJECT_DIR, "DATA_PROCESSED/eNet/",ver)
dir.create(dn.out, showWarnings = F)
fn.out = file.path(dn.out,"eNet_InputData_r1.txt")
