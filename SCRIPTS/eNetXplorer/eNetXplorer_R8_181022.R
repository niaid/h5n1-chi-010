...
data = read.table(file.path(path_infile,"eNet_InputData_r8.txt"), header=T, stringsAsFactors=F, sep="\t")
run_index = 8 # integer value in the 1:9 range
...
if (model=="SOMAscan") {
    predictor = c("sex","age",colnames(data)[-c(1:10)])
}
if (model=="SOMAscan_wCounts") {
    predictor = c("sex","age",colnames(data)[-c(1:10)]) # adds 7 columns with cell subset counts at the end
}
